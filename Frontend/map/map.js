...
function initMap(location) {
  ...
  map = new google.maps.Map(document.getElementById('map'), {
    mapTypeControl: true, // restricts map to satellite toggling
    // center: { lat: 40.76, lng: -73.983 },
    zoom: 7, // zoom in the map
    center: loc,// considering loc as center and accordingly giving paths
    mapTypeId: 'satellite'
  });
...
AutocompleteDirectionsHandler.prototype.route = function () {
  ...
  this.directionsService.route({
    ...
    if (status === 'OK') {
      var image1 = {
        url: "https://upload.wikimedia.org/wikipedia/commons/8/89/HD_transparent_picture.png",
      ...
      for (var k = 0, len = response.routes.length; k < len; k++) {
        ...
        div.innerHTML = '<ul class="msg"><li id=' + colour + '></li><li id=' +safetyi  + '></li><li id=' + distance + '></li><li id='+ duration + '></li></ul>' ; //print order
        
        document.getElementById(colour).innerHTML = "Route Number:  " + k + " is colored " +
          (color[k]);
          
        var message = 'Danger index on a  scale of 5:' + dindex[k];
        document.getElementById(safetyi).innerHTML = message;

        document.getElementById(distance).innerHTML = "Distance to be travelled : " +
          (response.routes[k].legs[0].distance.text);

        // Display the duration:
        document.getElementById(duration).innerHTML = "Total time duration for the travel: " +
